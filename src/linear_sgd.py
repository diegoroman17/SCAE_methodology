...
class LinearRegression(object):
    ...
    def __init__(self, input, n_in, n_out):
        ...
        # hyperplane-k
        self.y_given_x = T.transpose(T.dot(input, self.W) + self.b)
    ...
    def predictions(self, y):
        ...
        # the mean (across minibatch examples) of the elements in v,
        # i.e., the mean log-likelihood across the minibatch.
        return y, self.y_pred#T.mean(T.sum(T.pow(self.y_pred-y,2)))
        # end-snippet-2
    def errors(self, y):
...
def sgd_optimization_mnist(learning_rate=0.13, n_epochs=1000,
    ...
    #Training Data
    train_set_x = theano.shared(numpy.asarray([[0.,0.],[0.,0.1],[0.1,0.1],[0.2,0.1],[0.1,0.2],[0.25,0.1],[0.01,0.1],[0.5,0.1],[0.1,0.3],[0.2,0.6],[0.35,0.6],[0.1,0.4],[0.,0.3],[0.8,0.1],[0.9,0.1]],dtype=theano.config.floatX),borrow=True)
    train_set_y = theano.shared(numpy.asarray([0., 0.1, 0.2, 0.3, 0.3, 0.35, 0.11, 0.6, 0.4, 0.8, 0.95, 0.5, 0.3, 0.9, 1],dtype=theano.config.floatX),borrow=True)
    ...
    classifier = LinearRegression(input=x, n_in=2, n_out=1)
    ...
    pred_model = theano.function(
        inputs=[index],
        outputs=classifier.predictions(y),
        givens={
            x: valid_set_x[index * batch_size: (index + 1) * batch_size],
            y: valid_set_y[index * batch_size: (index + 1) * batch_size]
        }
