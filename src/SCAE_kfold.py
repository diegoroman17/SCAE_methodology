...
class SCAE(object):
    ...
    def build_finetune_functions_p(self, train_set_x, train_set_y, test_set_x,test_set_y, batch_size, learning_rate):
        ...
        # Create a function that scans the entire test set
        def test_result():
            return [test_result_i(i) for i in range(n_test_batches)]
        
        return train_fn, test_score, test_result
    ...
    def build_finetune_functions(self, datasets, batch_size, learning_rate):
        ...
        def valid_predictions():
            return [valid_predictions_i(i) for i in range(n_valid_batches)]

        # Create a function that scans the entire test set
        def test_predictions():
            return [test_predictions_i(i) for i in range(n_test_batches)]
...
def test_SCAE(finetune_lr=0.1, pretraining_epochs=100,
             ...
             path_results='/home/dcabrera/Dropbox/Seville/data/kfolds/resultsSCAE2/',
             pretrain=True,
             premodel=False,
             extend_verbose=True):
    """
    ...
    """
    print('load data')
    data,target = load_data2(dataset)
    ...
    for train_index, test_index in skf.split(data, target):
        ...
        train_set_x,train_set_y = shared_dataset(data[train_index],target[train_index])
        test_set_x,test_set_y = shared_dataset(data[test_index],target[test_index])
        ...
        if pretrain:
            if not premodel:
                ...
                try:
                    ...
                    if not os.path.isdir(path_results_test):
                        raise
                cost_each_layer = []
                for i in range(scae.n_layers):
                    ...
                    for epoch in range(pretraining_epochs):
                        ...
                        print('Pre-training layer %i, epoch %d, cost ' % (i, epoch),mean_value)
                ...
                for epoch in range(pretraining_epochs):
                    ...
                    mean_value = numpy.mean(c)
                    cost_each_epoch.append(mean_value)
                    print('Pre-training layer final, epoch %d, cost ' % epoch,mean_value)
                cost_each_layer.append(cost_each_epoch)

                save2file(path_results + 'pretrain_cost.pkl.gz', cost_each_layer)
                end_time = timeit.default_timer()
                # save the best model
                with open(path_results+'pretrained_model.pkl', 'wb') as f:
                    cPickle.dump(scae, f)
                print('The pretraining code for file ' + ' ran for %.2fm' % ((end_time - start_time) / 60.))
                # end-snippet-4
            with open(path_results + 'pretrained_model.pkl', 'rb') as f:
                scae = cPickle.load(f)
        else:
            scae = SCAE(
                numpy_rng=numpy_rng,
        ...
        train_fn_p, test_model_p, test_result_p = scae.build_finetune_functions_p(
        ...
        while (epoch < training_epochs) and (not done_looping):
            ...
            for minibatch_index in range(n_train_batches):
                ...
                if (itera + 1) % validation_frequency == 0:
                    validation_losses = test_model_p()
                    ...
                    if this_validation_loss < best_validation_loss:
                        ...
                        print(('     epoch %i, minibatch %i/%i, test error of '
                              ...
                              (epoch + 1, minibatch_index + 1, n_train_batches,
                               test_score * 100.))
                        # save the best model
                        #with open(path_results_test + 'predict_model.pkl', 'wb') as f:
                        #    cPickle.dump(scae, f)
                        save2file(path_results_test + 'test_result.pkl.gz', test_result_p())
                if patience <= itera:
                    done_looping = True
                    break
            epoch = epoch + 1
        save2file(path_results_test + 'train_cost.pkl.gz', (train_cost, val_cost, test_cost))
...
if __name__ == '__main__':
    test_SCAE(dataset='/home/dcabrera/Dropbox/Seville/data/hgbdcr_TF_ALL.pkl')
