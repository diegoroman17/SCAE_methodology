...
def test_SCAE(finetune_lr=0.13, pretraining_epochs=100,
             pretrain_lr=0.01, training_epochs=100,
             dataset='mnist.pkl.gz', batch_size=20,
             path_results='/home/dcabrera/Dropbox/Seville/data/results100l40lpt100/',
             extend_verbose=True):
    """
    ...
    train_fn_p, validate_model_p, test_model_p = scae.build_finetune_functions_p(
        datasets=datasets,
        batch_size=batch_size,
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    print(('     epoch %i, minibatch %i/%i, test error of '
                          ...
                          (epoch + 1, minibatch_index + 1, n_train_batches,
                           test_score * 100.))
                    # save the best model
                    with open('predict_model.pkl', 'w') as f:
                        cPickle.dump(scae, f)
    ...
    '''
    print '... finetunning the regression model'
    # early-stopping parameters
    patience = 10 * n_train_batches  # look as this many examples regardless
    patience_increase = 2.  # wait this much longer when a new best is
                            # found
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    print(('     epoch %i, minibatch %i/%i, test error of '
                          ...
                          (epoch + 1, minibatch_index + 1, n_train_batches,
                           test_score * 100.))
                    pred = val_model_pred()
                    t_label, p_label = numpy.concatenate(numpy.dstack(numpy.array(pred)))
                    t_label = numpy.hstack(t_label)
                    p_label = numpy.hstack(p_label)
                    p_label = p_label.reshape(t_label.shape)
